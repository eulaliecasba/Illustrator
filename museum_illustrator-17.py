...
def decide_placements(pdf_path, config, min_score=0.35, verbose=True):
    ...
    # Now (in the background build, not the timed scan) let Groq pick the best
    # object per section from its body text. This is where the slower per-
    # paragraph AI calls happen, paced against the free-tier rate limit.
    for sec in searchable:
        body = getattr(sec, "_body", "") or ""
        if body:
            better = _object_queries(body, period, use_llm=True)
            if better:
                sec.queries = better
