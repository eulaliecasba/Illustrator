...
object, inserts it as a full-page plate after that section begins, and cites it
...
class MetClient(MuseumClient):
    ...
    def search(self, query, limit=8):
        try:
            ...
            return []
        out = []
        for oid in (data.get("objectIDs") or [])[:limit * 3]:
            if len(out) >= limit:
                break
            try:
                obj = self._get(f"{self.BASE}/objects/{oid}")
            except requests.RequestException:
                continue
...
def find_artwork(section, clients, prefer, min_score, verbose):
    best = None
    for q in section.queries:
        for client in clients:
            for art in client.search(q):
                art.score = score_artwork(art, q, prefer)
                if verbose:
                    print(f"      [{art.score:+.2f}] {client.name}: {art.title[:70]}")
...
def build_clients(session, args):
    """Always-on keyless museums, plus keyed ones when a key is supplied.
    Keys may come from CLI flags or environment variables."""
    import os
    clients = [
        MetClient(session),
...
def cmd_build(args):
    ...
    sections, prefer = load_sections(args.config)
    ...
    for sec in found:
        print(f"  {sec.sid}  (p. {sec.page_index + 1})")
        if not sec.queries:
            print("      no queries set; skipping.")
            continue
        art = find_artwork(sec, clients, prefer, args.min_score, args.verbose)
        if art is None:
            print("      no suitable public-domain object found; skipping.")
            continue
        sec.artwork = art
        print(f"      -> {art.title}  [{art.museum}]  score {art.score:.2f}")
    ...
    print(f"\nWrote {out}  ({len(placed)} plates + credits page).")
