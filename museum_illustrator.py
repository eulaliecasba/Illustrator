...
def _llm_query(body, period):
    ...
    if not key:
        return None
    era = "" if period in ("any", None) else f" The text is from the {period} period; the object MUST be of that era and culture."
    prompt = (
        "You help illustrate a text with museum artwork. Read the passage and "
        "choose the single best thing to illustrate it with a real museum object."
        + era +
        " Prefer, in this order: (1) a specific artifact the passage explicitly "
        "names (e.g. a named vase, statue, or work); (2) a concrete object, "
        "creature, or artwork depicting a figure or scene from the passage, in "
        "the correct culture and period; avoid generic thematic matches (do NOT, "
        "for a Greek myth, return a later European painting of a similar theme). "
        "Reply with ONLY a short museum search query (2-6 words, English), naming "
        "the culture where helpful (e.g. 'Greek vase Hephaistos', 'Roman statue "
        "Aphrodite'), or the single word NONE if nothing concrete fits. "
        "No explanation.\n\nPassage:\n" + body[:1500])
...
def _object_queries(body, period="any"):
    ...
    mentions. If an LLM key is set, its judgment leads; otherwise (or on any
    failure) fall back to the curated vocabulary + frequency extraction."""
    queries, seen = [], []

    # 0. LLM judgment (smart), if available.
    g = _llm_query(body, period)
    if g:
