...
def find_artwork(section, clients, prefer, min_score, verbose):
    ...
    for q in section.queries:
        # Query all museums in parallel instead of one after another.
        results = []
        ...
        for art in results:
            ...
            if best is None or art.score > best.score:
                best = art
        # Stop as soon as one query yields a confident match.
...
def _looks_latin(text):
    ...
    # Real Latin narrative: several function words, or macron vowels present.
    ratio = latin_hits / len(words)
    return macron or latin_hits >= 4 or ratio >= 0.06


def _body_after(page, marker, doc, page_index):
    """Grab the text that follows a heading marker, spilling onto the next
    page if needed, so we can judge whether the section is real Latin."""
    full = page.get_text()
    idx = full.find(marker)
    body = full[idx + len(marker):] if idx >= 0 else full
    if len(body) < 60 and page_index + 1 < doc.page_count:
    ...
    for pno in range(doc.page_count):
        for blk in doc[pno].get_text("dict")["blocks"]:
            for line in blk.get("lines", []):
                ...
                # skips title pages, tables of contents, English intros, and
                # bare chapter-list headers, and keeps the run fast.
...
def cmd_build(args):
    ...
    def _do(sec):
        return sec, find_artwork(sec, clients, prefer, args.min_score, args.verbose)
